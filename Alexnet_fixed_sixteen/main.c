...
int16_t *ofmap3 = 0, *ofmap3p = 0;
int16_t *ofmap4 = 0, *ofmap4p = 0;
int16_t *ofmap5 = 0, *ofmap5p = 0;
int16_t *ofmap6 = 0;
int16_t *ofmap7 = 0;
int16_t *ofmap8 = 0;

int16_t *fmap1, *bias1, *fmap2, *bias2, *fmap3, *bias3, *fmap4, *bias4, *fmap5, *bias5;
int16_t *fmap6, *bias6, *fmap7, *bias7, *fmap8, *bias8;

// 파이썬 float을 읽어 16비트(Q11)로 변환
void load_quantized_weights(const char* filepath, int16_t* buffer, size_t num_elements) {
    ...
    // Q_FRAC(11)만큼 시프트 후 16비트 범위 내로 클리핑
    for(size_t i = 0; i < num_elements; i++) {
        int64_t q_val = (int64_t)roundf(temp_float[i] * (1 << Q_FRAC));
        if (q_val > 32767) q_val = 32767;
        if (q_val < -32768) q_val = -32768;
        buffer[i] = (int16_t)q_val;
    }
    free(temp_float);
}
...
int main()
{
	int data_set = 0;
	int i = 0;
	int16_t lval; int lidx;

	printf("----- AlexNet 16-BIT INT EMULATION Start -----\n\n");

	fmap1 = (int16_t*)calloc(M_C1 * C_C1 * R_C1 * S_C1, sizeof(int16_t));
	fmap2 = (int16_t*)calloc(M_C2 * C_C2 * R_C2 * S_C2, sizeof(int16_t));
	fmap3 = (int16_t*)calloc(M_C3 * C_C3 * R_C3 * S_C3, sizeof(int16_t));
	fmap4 = (int16_t*)calloc(M_C4 * C_C4 * R_C4 * S_C4, sizeof(int16_t));
	fmap5 = (int16_t*)calloc(M_C5 * C_C5 * R_C5 * S_C5, sizeof(int16_t));
	fmap6 = (int16_t*)calloc(M_C6 * C_C6 * R_C6 * S_C6, sizeof(int16_t));
	fmap7 = (int16_t*)calloc(M_C7 * C_C7 * R_C7 * S_C7, sizeof(int16_t));
	fmap8 = (int16_t*)calloc(M_C8 * C_C8 * R_C8 * S_C8, sizeof(int16_t));

	bias1 = (int16_t*)calloc(M_C1, sizeof(int16_t));
	bias2 = (int16_t*)calloc(M_C2, sizeof(int16_t));
	bias3 = (int16_t*)calloc(M_C3, sizeof(int16_t));
	bias4 = (int16_t*)calloc(M_C4, sizeof(int16_t));
	bias5 = (int16_t*)calloc(M_C5, sizeof(int16_t));
	bias6 = (int16_t*)calloc(M_C6, sizeof(int16_t));
	...
	for (data_set = 0; data_set < _data; data_set++){ 
		...
		ifmap1 = (int16_t *)calloc(H_C1*W_C1*C_C1, sizeof(int16_t));
		ofmap1 = (int16_t *)calloc(E_C1*F_C1*M_C1, sizeof(int16_t));
		ofmap1p= (int16_t *)calloc(E_P1*F_P1*M_C1, sizeof(int16_t));
		ofmap1pp=(int16_t *)calloc(H_C2*W_C2*M_C1, sizeof(int16_t));
		ofmap2 = (int16_t *)calloc(E_C2*F_C2*M_C2, sizeof(int16_t));
		ofmap2p= (int16_t *)calloc(E_P2*F_P2*M_C2, sizeof(int16_t));
		ofmap2pp=(int16_t *)calloc(H_C3*W_C3*M_C2, sizeof(int16_t));
		ofmap3 = (int16_t *)calloc(E_C3*F_C3*M_C3, sizeof(int16_t));
		ofmap3p= (int16_t *)calloc(H_C3*W_C3*M_C3, sizeof(int16_t));
		ofmap4 = (int16_t *)calloc(E_C4*F_C4*M_C4, sizeof(int16_t));
		ofmap4p= (int16_t *)calloc(H_C5*W_C5*M_C4, sizeof(int16_t));
		ofmap5 = (int16_t *)calloc(E_C5*F_C5*M_C5, sizeof(int16_t));
		ofmap5p= (int16_t *)calloc(H_C6*W_C6*M_C5, sizeof(int16_t));
		ofmap6 = (int16_t *)calloc(E_C6*F_C6*M_C6, sizeof(int16_t));
		ofmap7 = (int16_t *)calloc(E_C7*F_C7*M_C7, sizeof(int16_t));
		ofmap8 = (int16_t *)calloc(E_C8*F_C8*M_C8, sizeof(int16_t));

		// 입력 이미지도 16비트로 변환
		for (i = 0; i<H_C1*W_C1*C_C1; i++){
			int64_t q_val = (int64_t)roundf(input_image[data_set][i] * (1 << Q_FRAC));
			if (q_val > 32767) q_val = 32767;
			if (q_val < -32768) q_val = -32768;
			ifmap1[i] = (int16_t)q_val;
		}
		...
		printf("16-BIT Execution Time: %.4f seconds\n\n", elapsed_time);
